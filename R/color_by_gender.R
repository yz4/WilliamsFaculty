...
color_by_gender <- function() {
  ...
  palette("default")
  with(temp_data[temp_data$gender == "Male",], plot(age~index, col=1, xlim=c(0,405), ylim=c(26,81), main = "Williams Faculty Age by Gender"))
  with(temp_data[temp_data$gender == "Female",], points(age~index, col=2))
  legend("topright", inset = c(-0.25, 0.4), col = c(1,2), legend = c("Male","Female"), 
         bty = "n", pch = 1, title = "Gender", cex = 0.7)
}